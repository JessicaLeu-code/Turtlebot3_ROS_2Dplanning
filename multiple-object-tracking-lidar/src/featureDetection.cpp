...
vector<Vec2f> featureDetection::houghTransform(){
	...
	if (_lines.size() > 1){
		...
		kmeans(samples, 2, labels, TermCriteria(CV_TERMCRIT_ITER|CV_TERMCRIT_EPS, 1000, 0.001), _num_clusters, KMEANS_PP_CENTERS, centers );
...
Mat featureDetection::lineItr(Mat img, vector<Vec2f> lines, string name){
	...
	if (check){
		line(imgRGB,endPoints[0],pint,Scalar(0,0,255),_num_clusters);
		line(imgRGB,endPoints[2],pint,Scalar(0,0,255),_num_clusters);
	}	
	/////
