...
ros::Publisher objID_pub;
    ...
    cv::KalmanFilter KF6(stateDim,measDim,ctrlDim,CV_32F);
    cv::KalmanFilter KF7(stateDim,measDim,ctrlDim,CV_32F);
...
void KFT(const std_msgs::Float32MultiArray ccs)
{
    ...
    std::vector<cv::Mat> pred{KF0.predict(),KF1.predict(),KF2.predict(),KF3.predict(),KF4.predict(),KF5.predict(),KF6.predict(),KF7.predict()};
 //cout<<"Pred successfull\n";
  ...
  // cout<<"Got predictions"<<"\n";
    ...
    objID.resize(NUM_CLUSTERSplus1);//Allocate default elements so that [i] doesnt segfault. Should be done better
    ...
    for(int filterN=0;filterN<NUM_CLUSTERSplus1;filterN++)
    {
        std::vector<float> distVec;
        for(int n=0;n<NUM_CLUSTERSplus1;n++)
        {
    ...
    for(int clusterCount=0;clusterCount<NUM_CLUSTERSplus1;clusterCount++)
    {
        ...
        distMat[minIndex.first]=std::vector<float>(NUM_CLUSTERSplus1,10000.0);// Set the row to a high number.
  ...
  /* DEBUG
    ...
    */
    ...
    visualization_msgs::MarkerArray clusterMarkers;
     ...
     for (int i=0;i<NUM_CLUSTERSplus1;i++)
     {
    ...
    float meas6[2]={cc[6].at(0),cc[6].at(1)};
    float meas7[2]={cc[7].at(0),cc[7].at(1)};
    ...
    cv::Mat meas6Mat=cv::Mat(2,1,CV_32F,meas6);
    cv::Mat meas7Mat=cv::Mat(2,1,CV_32F,meas7);
...
if (!(meas6[0]==0.0f || meas6[1]==0.0f))
    Mat estimated6 = KF6.correct(meas6Mat);
if (!(meas7[0]==0.0f || meas7[1]==0.0f))
    Mat estimated7 = KF7.correct(meas7Mat);
...
void publish_cloud(ros::Publisher& pub, pcl::PointCloud<pcl::PointXYZ>::Ptr cluster){
  ...
  clustermsg->header.frame_id = "/base_scan";
...
{   
  ...
  // Could be made generic by creating a Kalman Filter only when a new object is detected  
    ...
    KF6.transitionMatrix = (Mat_<float>(4, 4) << dx,0,1,0,   0,dy,0,1,  0,0,dvx,0,  0,0,0,dvy);
    KF7.transitionMatrix = (Mat_<float>(4, 4) << dx,0,1,0,   0,dy,0,1,  0,0,dvx,0,  0,0,0,dvy);
    ...
    setIdentity(KF6.processNoiseCov, Scalar::all(sigmaP));
    setIdentity(KF7.processNoiseCov, Scalar::all(sigmaP));
...
// Process the point cloud
  ...
  for(it = cluster_indices.begin(); it != cluster_indices.end(); ++it) {
    ...
    while (cluster_vec.size() < NUM_CLUSTERSplus1){
      ...
      cluster_vec.push_back(empty_cluster);
        ...
        while (clusterCentroids.size()<NUM_CLUSTERSplus1)
    {
      pcl::PointXYZ centroid;
      centroid.x=0.0;
      centroid.y=0.0;
     ...
     // Set initial state
     KF6.statePre.at<float>(0)=clusterCentroids.at(6).x;
     KF6.statePre.at<float>(1)=clusterCentroids.at(6).y;
     KF6.statePre.at<float>(2)=0;// initial v_x
     KF6.statePre.at<float>(3)=0;//initial v_y

     // Set initial state
     KF7.statePre.at<float>(0)=clusterCentroids.at(7).x;
     KF7.statePre.at<float>(1)=clusterCentroids.at(7).y;
...
//cout<<"PCL extract successfull\n";
  ...
  for(it = cluster_indices.begin(); it != cluster_indices.end(); ++it)
   ...
   // cout<<"6 clusters initialized\n";
    ...
    KFT(cc);
    int i=0;
    bool publishedCluster[NUM_CLUSTERSplus1];
    for(auto it=objID.begin();it!=objID.end();it++)
...
// Create a ROS subscriber for the input point cloud
  ...
  pub_cluster7 = nh.advertise<sensor_msgs::PointCloud2> ("cluster_7", 1);
